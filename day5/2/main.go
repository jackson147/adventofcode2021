...
func main() {
	lines, err := readLines("/home/rich/git/advent2021/day5/2/real.txt")
	check(err)
	...
	graphLinesDiagonal := make([]graphLine, 0)
	...
	for _, graphLine := range graphLines {
		...
		if vertical || horizontal {
			...
			//The distance to walk each point will be the same as per the rules.
			//Calculate for X then use for Y (loop, so base index 0)
			...
			//Calculate the direction to take for X & Y, postive or negative
			xDirection := 1
			yDirection := 1
			...
			graphLine.points = points
			graphLinesDiagonal = append(graphLinesDiagonal, graphLine)
			graphLinesZeroAngle = append(graphLinesZeroAngle, graphLine)
